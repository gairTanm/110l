...
fn main() {
    ...
    // unimplemented!();
    let get_target_result = ps_utils::get_target(target)
        .expect("issue with ps or pgrep");


    let process = match get_target_result {
        Some(result) => {
    ...
    let children = ps_utils::get_child_processes(process.pid).expect("no child process");
